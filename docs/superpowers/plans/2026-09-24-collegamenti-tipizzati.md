...
4. **La regola degli stereotipi vive solo nel gesto.** La spec non chiede di validare una classe che diventa interfaccia dopo il collegamento, quindi la regola «solo `class` e `abstract`» sta in `connectAcross` e non nel modello.
