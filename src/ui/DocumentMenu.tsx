...
/** Rinomina dalla UI: un nome vuoto non è un comando, e il campo torna al nome di prima. */
const commitName = (value: string): boolean => {
  const recipe = renameDocument(value)
  return recipe !== null && documentStore.getState().dispatch(recipe)
}
...
export function DocumentMenu() {
  ...
  /** Chiudendosi il menu ridà il fuoco al trigger: se si sta rinominando lo ruberebbe al campo appena montato. */
  const renamingRef = useRef(false)
  const [importOpen, setImportOpen] = useState(false)
  const [textExportOpen, setTextExportOpen] = useState(false)

  const refreshRecent = () => void documentDb.listRecent().then(setRecent, () => setRecent([]))
  ...
  const removeRecent = (id: string) => void documentIo.removeRecent(id).then(refreshRecent)
  ...
  return (
    <>
      {renaming ? (
        // Il blur del campo arriva prima a CommitInput, che committa, poi qui, che chiude.
        <span onBlur={stopRename}>
          <CommitInput
            key={name}
            value={name}
            onCommit={commitName}
              ...
              <>
                ...
                {recent.map((r) => (
                  <DropdownMenuItem
                    ...
                    disabled={r.id === docId}
                    className="group"
                    onSelect={() => void documentIo.openRecent(r.id)}
                    onKeyDown={(e) => {
                      if (e.key === "Delete" || e.key === "Backspace") removeRecent(r.id)
                    }}
                  >
                    <span className="truncate">{r.name}</span>
                    <span className="ml-auto pl-3 text-xs text-muted-foreground">{when.format(r.updatedAt)}</span>
                    {/* Fuori dal tab order: nel menu si naviga con le frecce, e da tastiera c'è Canc sulla voce. */}
                    <button
                      type="button"
                      tabIndex={-1}
                      aria-label={`Elimina «${r.name}» dal browser`}
                      title="Elimina dal browser (il file su disco resta)"
                      className="-my-1 -mr-1 rounded p-1 opacity-0 group-hover:opacity-100 group-data-highlighted:opacity-100 hover:text-destructive"
                      // Senza, il click risalirebbe alla voce e aprirebbe il documento che si vuole eliminare.
                      onClick={(e) => {
                        e.stopPropagation()
                        removeRecent(r.id)
                      }}
